...
- **Import the author's own database exports (P7).** If `source/<folder>/_exports/` holds files, run `import_external.py` and concatenate its records into the stream before dedupe. **Never query Scopus, Web of Science, Embase or CENTRAL yourself** — an agent using someone's institutional subscription breaks their licence; you read the file they exported, nothing more. A file without a manifest entry is not imported, and a shortfall between `n_reported` and records parsed means the export was truncated by the platform's download cap — surface it at Gate 2b rather than treating the cap as the result set.
- **Widen with the three no-key APIs (P7).** Europe PMC, OpenAlex and Semantic Scholar have no MCP here; call them with `WebFetch` and put the **exact URL** in the ledger's `call` column. Confirm every hit's PMID/DOI in PubMed before it enters the store. Space unauthenticated calls to one host (~3/s at NCBI, ~1/s at Semantic Scholar); on a 429, slow that host for the rest of the run — nothing in this harness throttles for you, so this is your discipline, not a mechanism.
