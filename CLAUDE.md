...
`medical-review-orchestrator` skill. It runs a **4-agent** team (scoping-retriever → appraiser →
writer → verifier) plus a **conditional** quality-coach (full/high-stakes only); the lead runs the
lessons-learned loop itself. A single factual medical question is answered directly.

**Three human gates:** (1) Phase-0 scope confirm; (2) Phase-3 **Research Map** — the **hard gate**, which
also carries the corpus + full-text/source review formerly split out as Gate 2b (L-022); (3) Gate 4b
post-appraisal (GRADE + contradictions + assumptions, L-014). All three run on every effort level (incl.
`tiny`); the Research Map hard gate has no small-scope exception. The team STOPS and waits for explicit
user approval at each.
...
| 2026-07-12 | **Lightening pass (3 steps, no invariant sacrificed)** — trimmed harness *ceremony* while keeping every load-bearing check (deterministic scripts, writer≠verifier, hard gate, Law 1). **(1) Lessons:** added a `Scope:` tag (`universal`/`vi-language`/`cardiology-ep`); inject now loads `Scope`∩`Role` only, never the whole store, so it stays light as it grows. Consolidated the gate-discipline cluster (L-024/L-030/L-041/L-042/L-044→**L-014**) and Vietnamese fluency (L-037→**L-033**); tagged the two truly cardiac lessons (L-018/L-020) `cardiology-ep`. **(2) Agents 7→4:** merged `research-strategist`+`evidence-retriever`→**`scoping-retriever`** (one head, no protocol→search hand-off); demoted `quality-coach` to a **conditional** pass (full/high-stakes only; declared skip `04b_coach_skip.md` otherwise, L-025); folded `lessons-curator` into the lead (it is file-load+append, no specialist reasoning). **(3) Gates 4→3:** folded **Gate 2b into the Research Map** so the user reviews the corpus/source base once, not at two adjacent stops (Research Map part 6 now carries corpus size + abstract-only-HIGH flags + unavailable-source options, L-022). Updated constitution, orchestrator SKILL, CLAUDE.md, README, audit.md, lessons + curator/coach agents. | CLAUDE.md, README.md, constitution.md, orchestrator SKILL.md, audit.md, lessons.md, lessons-learned SKILL.md, agents/ (scoping-retriever new; research-strategist/evidence-retriever/lessons-curator removed; quality-coach updated) | Cut ~40% of vessel/coordination weight (fewer agents, fewer gate stops, lighter lesson injection) without touching the mechanisms an LLM can't talk past |
